...
x = number - 50 #we set the minimum guess value to be 50 less than the number in order to prevent the guess generated by the computer to be extremely low
y = number + 50 #we set the maximum guess value to be 50 more than the number in order to prevent the guess generated by the computer to be extremely high
#I set these variables like this so that the player can use any numbers and not be restricted to a certain range

#The following command is for one of the optional extensions for when the user types in a value other than l,h,c
...
keywordsCorrect = ["right", "correct", "got it", "did it", "congrats", "congratulation", "you won", "I lost", "you win", "I lose", "perfect"]
